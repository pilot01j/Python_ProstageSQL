...
# predefine de type of value that wil be shared or received
...
WHERE polls.id = %s;"""
SELECT_LATEST_POLL = """SELECT * FROM pols
JOIN options ON polls.id = options.poll_id
WHERE polls.id = (
...
SELECT_POLL_VOTE_DETAILS = """SELECT
    ...
    FROM options
    LEFT JOIN votes ON option_id = votes.option_id
